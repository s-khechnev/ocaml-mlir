...
let () = register_and_load_upstream_dialects IR.Context.global_ctx
...
let populate_loop_body ctx loop_body loc func_body =
  ...
  (* lhs *)
  let load_lhs_state = OperationState.get "memref.load" loc in
  ...
  (* rhs *)
  let load_rhs_state = OperationState.get "memref.load" loc in
  ...
  (* addf *)
  let add_state = OperationState.get "arith.addf" loc in
  ...
  (* store state *)
  let store_state = OperationState.get "memref.store" loc in
...
let make_and_dump_add ctx loc =
  ...
  let func_body_arg_locs = [ loc; loc ] in
  let func_region = Region.create () in
  let func_body = Block.create func_body_arg_types func_body_arg_locs in
  ...
  let func_attrs =
    [ Attribute.name "function_type" func_type_attr
    ; Attribute.name "sym_name" func_name_attr
    ]
  in
  let func_state = OperationState.get "func.func" loc in
  ...
  let const_zero_state = OperationState.get "arith.constant" loc in
  ...
  let dim_state = OperationState.get "memref.dim" loc in
  ...
  let loop_body = Block.create [ index_type ] [ loc ] in
  ...
  let const_one_state = OperationState.get "arith.constant" loc in
  ...
  let ret_state = OperationState.get "func.return" loc in
  let ret = Operation.create ret_state in
  Block.append_owned_operation func_body ret;
  let _ = IR.Operation.verify @@ IR.Module.operation module_op in
  module_op
...
let%expect_test _ =
  with_context (fun ctx ->
    ...
    let loc = IR.Location.unknown ctx in
    IR.(Operation.dump @@ Module.operation @@ make_and_dump_add ctx loc));
  [%expect
    {|
     module {
...
let build_with_insertions_and_print ctx =
  ...
  let () = IR.Context.set_allow_unregistered_dialects ctx true in
  ...
  let i5 = BuiltinTypes.Integer.get ctx 5 in
  let block1 = Block.create [ i1 ] [ loc ] in
  let block2 = Block.create [ i2 ] [ loc ] in
  let block3 = Block.create [ i3 ] [ loc ] in
  let block4 = Block.create [ i4 ] [ loc ] in
  let block5 = Block.create [ i5 ] [ loc ] in
  ...
  Region.insert_owned_block_before region block3 block5;
  ...
  let op8_state = OperationState.get "dummy.op8" loc in
  ...
  let op8 = Operation.create op8_state in
  ...
  Block.append_owned_operation block5 op8;
  IR.Block.detach block5;
  IR.Block.destroy block5;
  Operation.dump op;
  Operation.destroy op;
  IR.Context.set_allow_unregistered_dialects ctx false


let%expect_test _ =
  with_context (fun ctx ->
    register_and_load_upstream_dialects ctx;
    build_with_insertions_and_print ctx);
  [%expect
    {|
...
(*
   ...
   Printf.printf "\n%!";
   (* Ranked tensor type *)
   let ranked_tensor = BuiltinTypes.Tensor.ranked shape f32 BuiltinAttributes.null in
   ...
   Printf.printf "\n%!";
   (* MemRef type *)
   let memspace2 = BuiltinAttributes.Integer.get (BuiltinTypes.Integer.get ctx 64) 2 in
   let memref = BuiltinTypes.MemRef.contiguous f32 shape memspace2 in
   Type.dump memref;
   Printf.printf "\n%!";
   (* Unranked MemRef type *)
   let memspace4 = BuiltinAttributes.Integer.get (BuiltinTypes.Integer.get ctx 64) 4 in
   let unranked_memref = BuiltinTypes.MemRef.unranked f32 memspace4 in
   ...
   let floats = [ 0.; 1. ] in
   let doubles = [ 0.; 1. ] in
   let encoding = BuiltinAttributes.null in
   let bool_elements =
   BuiltinAttributes.Dense.Elements.bool_get
   BuiltinTypes.(Tensor.ranked shp Integer.(get ctx 1) encoding)
   bools
   in
   let uint32_elements =
   BuiltinAttributes.Dense.Elements.uint32_get
   BuiltinTypes.(Tensor.ranked shp Integer.(unsigned ctx 32) encoding)
   uints32
   in
   let int32_elements =
   BuiltinAttributes.Dense.Elements.int32_get
   BuiltinTypes.(Tensor.ranked shp Integer.(get ctx 32) encoding)
   ints32
   in
   let uint64_elements =
   BuiltinAttributes.Dense.Elements.uint64_get
   BuiltinTypes.(Tensor.ranked shp Integer.(unsigned ctx 64) encoding)
   uints64
   in
   let int64_elements =
   BuiltinAttributes.Dense.Elements.int64_get
   BuiltinTypes.(Tensor.ranked shp Integer.(get ctx 64) encoding)
   ints64
   in
   let float_elements =
   BuiltinAttributes.Dense.Elements.float_get
   BuiltinTypes.(Tensor.ranked shp Float.(f32 ctx) encoding)
   floats
   in
   let double_elements =
   BuiltinAttributes.Dense.Elements.double_get
   BuiltinTypes.(Tensor.ranked shp Float.(f64 ctx) encoding)
   doubles
   in
   (* is dense *)
   assert (BuiltinAttributes.Dense.Elements.is_dense bool_elements);
   assert (BuiltinAttributes.Dense.Elements.is_dense uint32_elements);
   assert (BuiltinAttributes.Dense.Elements.is_dense int32_elements);
   assert (BuiltinAttributes.Dense.Elements.is_dense uint64_elements);
   assert (BuiltinAttributes.Dense.Elements.is_dense int64_elements);
   assert (BuiltinAttributes.Dense.Elements.is_dense float_elements);
   assert (BuiltinAttributes.Dense.Elements.is_dense double_elements);
   (* Get value *)
   assert (BuiltinAttributes.Dense.Elements.(bool_value bool_elements 1) = true);
   assert (BuiltinAttributes.Dense.Elements.(uint32_value uint32_elements 1) = 1);
   assert (BuiltinAttributes.Dense.Elements.(int32_value int32_elements 1) = 1);
   assert (BuiltinAttributes.Dense.Elements.(uint64_value uint64_elements 1) = 1);
   assert (BuiltinAttributes.Dense.Elements.(int64_value int64_elements 1) = 1);
   assert (
   Float.(abs (BuiltinAttributes.Dense.Elements.(float_value float_elements 1) -. 1.))
   < 1E-6);
   assert (
   Float.(abs (BuiltinAttributes.Dense.Elements.(double_value double_elements 1) -. 1.))
   < 1E-6);
   (* dump *)
   ...
   (* SPLAT *)
   let bool_elements =
   BuiltinAttributes.Dense.Elements.bool_splat_get
   BuiltinTypes.(Tensor.ranked shp Integer.(get ctx 1) encoding)
   true
   in
   let uint32_elements =
   BuiltinAttributes.Dense.Elements.uint32_splat_get
   BuiltinTypes.(Tensor.ranked shp Integer.(unsigned ctx 32) encoding)
   1
   in
   let int32_elements =
   BuiltinAttributes.Dense.Elements.int32_splat_get
   BuiltinTypes.(Tensor.ranked shp Integer.(get ctx 32) encoding)
   1
   in
   let uint64_elements =
   BuiltinAttributes.Dense.Elements.uint64_splat_get
   BuiltinTypes.(Tensor.ranked shp Integer.(unsigned ctx 64) encoding)
   1
   in
   let int64_elements =
   BuiltinAttributes.Dense.Elements.int64_splat_get
   BuiltinTypes.(Tensor.ranked shp Integer.(get ctx 64) encoding)
   1
   in
   let float_elements =
   BuiltinAttributes.Dense.Elements.float_splat_get
   BuiltinTypes.(Tensor.ranked shp Float.(f32 ctx) encoding)
   1.
   in
   let double_elements =
   BuiltinAttributes.Dense.Elements.double_splat_get
   BuiltinTypes.(Tensor.ranked shp Float.(f64 ctx) encoding)
   1.
   in
   (* is dense *)
   ...
   assert (BuiltinAttributes.Dense.Elements.is_dense double_elements);
   (* is splat *)
   assert (BuiltinAttributes.Dense.Elements.is_splat bool_elements);
   assert (BuiltinAttributes.Dense.Elements.is_splat uint32_elements);
   assert (BuiltinAttributes.Dense.Elements.is_splat int32_elements);
   assert (BuiltinAttributes.Dense.Elements.is_splat uint64_elements);
   assert (BuiltinAttributes.Dense.Elements.is_splat int64_elements);
   assert (BuiltinAttributes.Dense.Elements.is_splat float_elements);
   assert (BuiltinAttributes.Dense.Elements.is_splat double_elements);
   (* Get value *)
   assert (BuiltinAttributes.Dense.Elements.(bool_splat_value bool_elements) = 1);
   assert (BuiltinAttributes.Dense.Elements.(uint32_splat_value uint32_elements) = 1);
   assert (BuiltinAttributes.Dense.Elements.(int32_splat_value int32_elements) = 1);
   assert (BuiltinAttributes.Dense.Elements.(uint64_splat_value uint64_elements) = 1);
   assert (BuiltinAttributes.Dense.Elements.(int64_splat_value int64_elements) = 1);
   assert (
   Float.(
   abs (BuiltinAttributes.Dense.Elements.(float_splat_value float_elements) -. 1.))
   < 1E-6);
   assert (
   Float.(
   abs (BuiltinAttributes.Dense.Elements.(double_splat_value double_elements) -. 1.))
   < 1E-6);
   (* dump *)
   Attribute.dump bool_elements;
   Attribute.dump uint32_elements;
   ...
   let indices =
   BuiltinAttributes.Dense.Elements.int64_get
   BuiltinTypes.(Tensor.ranked [| 2 |] Integer.(get ctx 64) encoding)
   [ 4; 7 ]
   in
   let values =
   BuiltinAttributes.Dense.Elements.float_get
   BuiltinTypes.(Tensor.ranked [| 2 |] Float.(f32 ctx) encoding)
   floats
   in
   let sparse =
   BuiltinAttributes.Sparse.create
   BuiltinTypes.(Tensor.ranked [| 2 |] Float.(f32 ctx) encoding)
   indices
   values
   in
   ...
   let affine_map = AffineMap.zero_result ctx 3 2 in

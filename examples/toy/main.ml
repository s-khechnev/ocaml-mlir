...
let config = { action = None; enable_opt = false }
...
let () =
  ...
  in
  Arg.parse args (fun _ -> failwith "wrong arg") ("Use -emit to " ^ emit_doc);
  match config.action with
  | None -> ()
  | DumpAST -> failwith "not implemented"
  | _ ->
    let open Mlir in
    ...
    (match config with
     | { action = DumpMLIR; enable_opt = false } ->
